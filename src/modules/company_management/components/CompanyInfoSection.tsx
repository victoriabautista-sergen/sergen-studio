...
const CompanyInfoSection = ({ company }: { company: Company }) => {
  const { toast } = useToast();
  const queryClient = useQueryClient();

  const contractInfo = company.contract_info ?? {};
  const energyInfo = company.energy_supply_info ?? {};
    ...
    <Card>
      ...
      <CardContent>
        <div className="grid sm:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            <Label>Email</Label>
            <Input type="email" value={formEmail} onChange={(e) => setFormEmail(e.target.value)} placeholder="contacto@empresa.com" />
          </div>
          <div className="space-y-2">
            <Label>Teléfono</Label>
            <Input value={formPhone} onChange={(e) => setFormPhone(e.target.value)} placeholder="+51 999 999 999" />
          </div>
          <div className="space-y-2 sm:col-span-2">
            <Label>Ingeniero asignado</Label>
            <Input value={formEngineer} onChange={(e) => setFormEngineer(e.target.value)} placeholder="Nombre del ingeniero" />
          </div>
        </div>
