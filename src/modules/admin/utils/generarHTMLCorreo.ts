...
export function generarHTMLCorreo(d: EmailTemplateData): string {
  ...
  const chartBlock = d.chartPreviewUrl
    ? `<tr><td style="padding:12px 0">
        <p style="margin:0 0 8px;padding:0 24px;font-size:13px;font-weight:700;color:#374151">Pronóstico de Demanda</p>
        <div style="width:100%;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;background:#fff;position:relative;height:320px">
          <iframe src="${d.chartPreviewUrl}" style="width:800px;height:420px;border:none;transform:scale(0.75);transform-origin:0 0;display:block;position:absolute;top:0;left:0" frameborder="0" scrolling="no"></iframe>
        </div>
       </td></tr>`
